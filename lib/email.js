...
export async function enviarTicketCompra(participante, rifa, boletos, options = {}) {
  ...
  const numerosRows = (() => {
    const list = boletos || [];
    if (list.length === 0) {
      return `<tr><td align="center" style="padding:20px 12px;font-family:Arial,sans-serif;font-size:14px;color:${C_TEXT};opacity:0.75;">Pendiente de asignación</td></tr>`;
    }
    const gridRows = [];
