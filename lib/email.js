...
export async function enviarConfirmacionAdmin(participante, rifa) {
  if (!resendNotifyAdminEnabled()) return null;
  const to =
    process.env.NODE_ENV === "development" && process.env.RESEND_TEST_EMAIL
      ? process.env.RESEND_TEST_EMAIL
  ...
  const resend = getResend();
  const bodyHtml = `
<p style="margin:0 0 12px;"><strong>Nombre:</strong> ${participante?.nombre || "—"}</p>
<p style="margin:0 0 12px;"><strong>Email:</strong> ${participante?.email || "—"}</p>
<p style="margin:0 0 12px;"><strong>Teléfono:</strong> ${participante?.telefono || "—"}</p>
<p style="margin:0 0 12px;"><strong>Ciudad:</strong> ${participante?.ciudad || "—"}</p>
<p style="margin:0 0 12px;"><strong>Cédula:</strong> ${participante?.cedula || "—"}</p>
<p style="margin:0 0 12px;"><strong>Plan:</strong> ${rifa?.nombre || "—"}</p>
<p style="margin:0 0 12px;"><strong>Claves:</strong> ${participante?.cantidad_boletos ?? 0}</p>
<p style="margin:0 0 12px;"><strong>Total:</strong> ${formatPrecio(participante?.total_pagado)}</p>
<p style="margin:0 0 12px;"><strong>Fecha:</strong> ${formatFechaParticipante(participante?.created_at)}</p>
<p style="margin:0;"><strong>Estado:</strong> ${participante?.estado_pago || "aprobado"}</p>
`;

  const { data, error } = await resend.emails.send({
    from: resendFromAddress(),
    to: [to],
    subject: "Club Gómez — Nueva venta",
    html: emailShell({ title: "Nueva venta registrada", bodyHtml }),
  });
  if (error) throw new Error(error.message);
  return data;
}
...
export async function enviarNotificacionBendecido(
  ...
  const { data, error } = await resend.emails.send({
    from: resendFromAddress(),
    to: [to],
    subject: "Club Gómez — Número bendecido",
    html: emailShell({
      title: "Número bendecido asignado",
      bodyHtml: `
<p style="margin:0 0 12px;">Un miembro recibió un número de la lista bendecida.</p>
<p style="margin:0 0 8px;"><strong>Plan:</strong> ${rifa?.nombre || "—"}</p>
<p style="margin:0 0 8px;"><strong>Nombre:</strong> ${participante?.nombre || "—"}</p>
<p style="margin:0 0 8px;"><strong>Email:</strong> ${participante?.email || "—"}</p>
<p style="margin:0 0 8px;"><strong>Teléfono:</strong> ${participante?.telefono || "—"}</p>
<p style="margin:0;"><strong>Números:</strong> ${nums || "—"}</p>
`,
    }),
  });
  if (error) throw new Error(error.message);
