...
export async function crearPreferencia(participante, rifa) {
  ...
  const response = await preference.create({
    body: {
      ...
      back_urls: {
        success: `${process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000"}/confirmacion`,
        failure: `${process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000"}/comprar`,
        pending: `${process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000"}/confirmacion`,
      },
