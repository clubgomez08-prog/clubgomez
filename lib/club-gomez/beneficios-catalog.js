...
export const CATALOGO_PREMIOS = [
  ...
  {
    slug: "moto-110",
    nombre: "Motocicleta Special 110 X",
    imagenKey: "beneficio-moto-110",
    labelPlaceholder: "Special 110 X",
    destacado: true,
    fechasIso: ["2026-10-17"],
  },
  {
    slug: "moto-150",
    nombre: "Motocicleta DR 150",
    imagenKey: "beneficio-moto-150",
    labelPlaceholder: "DR 150",
    destacado: true,
...
export function agruparPremiosDesdeFilas(filas = []) {
  ...
  for (const row of filas) {
    const slugRaw = row.slug === "moto-125" ? "moto-150" : row.slug;
    const catalog =
      CATALOGO_PREMIOS.find((c) => c.slug === slugRaw) ||
      CATALOGO_PREMIOS.find((c) => c.nombre === row.premio) ||
      CATALOGO_PREMIOS.find((c) => c.slug === row.slug);

    const nombre = catalog?.nombre || String(row.premio || "")
      .replace(/\bAKT\b/gi, "")
      .replace(/\s{2,}/g, " ")
      .trim() || "Premio";
    const imagenKey = catalog?.imagenKey || row.imagen_key || null;
    const slug = catalog?.slug || slugRaw || row.id;
  ...
  // Grid incluye todas las tarjetas (también las motos, aparte de la cápsula)
  const grid = items.filter(
    (i) => i.slug !== "nevera" && Boolean(i.imagenKey)
  );
...
export const BENEFICIOS_MES = CATALOGO_PREMIOS.map((c) => ({
  id: c.slug,
  nombre: c.nombre,
