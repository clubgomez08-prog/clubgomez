...
/** Evita doble Purchase en Strict Mode / re-renders antes de sessionStorage */
const purchaseFiredInMemory = new Set();
...
/**
 * Iniciar checkout — clic en plan / llegada al formulario.
 * user opcional: Advanced Matching si hay datos.
 */
...
/** Una sola vez por plan y pestaña (evita doble IC clic + formulario). */
export function trackInitiateCheckoutOnce(plan, user) {
  const id = plan?.id || "esencial";
  try {
    const key = `cg_meta_ic_${id}`;
    if (sessionStorage.getItem(key) === "1") return false;
    sessionStorage.setItem(key, "1");
  } catch {
    /* private mode: igual dispara */
  }
  trackInitiateCheckout(plan, user);
  return true;
}

/**
 * Dispara IC y luego navega. Da un instante al pixel antes del unload
 * (si no, Meta a menudo no registra el evento del clic).
 */
export function trackInitiateCheckoutThenGo(plan, navigate, user) {
  trackInitiateCheckoutOnce(plan, user);
  ...
  window.setTimeout(() => {
    try {
      ...
      console.warn("[meta-pixel] navigate:", err?.message || err);
    }
  }, 280);
}
...
export function trackPurchase(plan, { orderId, user } = {}) {
  ...
  if (oid) {
    ...
    if (purchaseFiredInMemory.has(oid)) return;
    try {
      if (sessionStorage.getItem(key) === "1") {
        purchaseFiredInMemory.add(oid);
        return;
      }
      sessionStorage.setItem(key, "1");
    } catch {
