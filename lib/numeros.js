...
export async function asignarNumerosParticipante(rifaId, participanteId, cantidad) {
  ...
  const bendecidosParsed = parseNumerosBendecidos(rifaRow.numeros_bendecidos);
  const reservados = new Set(
    bendecidosParsed.filter((x) => x.bloqueado).map((x) => x.numero)
  );
  ...
  while (true) {
    ...
    allNums = allNums.concat(data.map((r) => r.numero).filter(Boolean));
    if (data.length < PAGE_SIZE) break;
    from += PAGE_SIZE;
  }

  const usados = new Set(allNums);
